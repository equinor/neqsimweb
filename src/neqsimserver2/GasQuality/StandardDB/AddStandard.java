...
public class AddStandard extends AbstractPageBean {
    ...
    private void _init() throws Exception {
        ...
        documentFullDataProvider.setCachedRowSet((javax.sql.rowset.CachedRowSet) getValue("#{SessionBean1.standardsFullRowSet}"));
    }
    ...
    private CachedRowSetDataProvider documentFullDataProvider = new CachedRowSetDataProvider();

    public CachedRowSetDataProvider getDocumentFullDataProvider() {
        return documentFullDataProvider;
    }

    public void setDocumentFullDataProvider(CachedRowSetDataProvider crsdp) {
        this.documentFullDataProvider = crsdp;
    }
    ...
    public void destroy() {
        ...
        documentFullDataProvider.close();
    }
    ...
    public String uploadButton_action() {
        ...
        try {
            if (!documentNameTextField.getText().toString().equals("")) {
                documentFullDataProvider.cursorLast();
                RowKey rk = documentFullDataProvider.appendRow();

                documentFullDataProvider.setValue("NAME", rk, documentNameTextField.getText().toString());
                
                try {
                    documentFullDataProvider.setValue("TEXT", rk, (documentTextInfo.getText().toString()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                try {
                    documentFullDataProvider.setValue("LINK", rk, (httpTextField.getText().toString()));
                } catch (Exception e) {
                    e.printStackTrace();
                }

                documentFullDataProvider.setValue("ARTICLEID", rk, standardTypeDropDown.getValue());
                documentFullDataProvider.setValue("ARTICLETYPE", rk, standardTypeDropDown.getValue());
                documentFullDataProvider.setValue("SUBJECT", rk, standardSubjectDropDown1.getValue());

                String filename = "no name";
                if (fileUpload1.getUploadedFile().getOriginalName().lastIndexOf("\\") > 0) {
                    documentFullDataProvider.setValue("FILETYPE", rk, fileUpload1.getUploadedFile().getContentType());
                    filename = fileUpload1.getUploadedFile().getOriginalName().substring(fileUpload1.getUploadedFile().getOriginalName().lastIndexOf("\\") + 1);
                    documentFullDataProvider.setValue("FILEID", rk, filename);
                    documentFullDataProvider.setValue("FILE", rk, fileUpload1.getUploadedFile().getBytes());
                }

                documentFullDataProvider.commitChanges();
                documentFullDataProvider.refresh();
            } else {
